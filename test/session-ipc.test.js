...
function createHarness(overrides = {}) {
  ...
  const runtime = registerSessionIpc({
    ...
    clipboard: overrides.clipboard || {
      writeText: (value) => calls.push(["clipboard.writeText", value]),
    },
  });
...
test("session IPC registers owned channels and disposes them", () => {
  ...
  assert.deepStrictEqual([...ipcMain.handlers.keys()].sort(), [
    ...
    "session-hud:choose-user-input-option",
  ...
  assert.deepStrictEqual([...ipcMain.listeners.keys()].sort(), [
    ...
    "session-hud:open-user-input-session",
...
test("session HUD choice copies the label and opens the matching Codex session", async () => {
  const { ipcMain, calls } = createHarness();

  const result = await ipcMain.invoke("session-hud:choose-user-input-option", {
    sessionId: "codex:thread-1",
    label: "  Keep current version\n",
  });
  ipcMain.send("session-hud:open-user-input-session", "codex:thread-1");

  assert.deepStrictEqual(result, { status: "ok", copied: true, focused: true });
  assert.deepStrictEqual(calls, [
    ["clipboard.writeText", "Keep current version"],
    ["focusSession", "codex:thread-1", {
      requestSource: "hud-user-input-choice",
      allowCodexDesktopFallback: true,
    }],
    ["focusSession", "codex:thread-1", {
      requestSource: "hud-user-input",
      allowCodexDesktopFallback: true,
    }],
  ]);
});
