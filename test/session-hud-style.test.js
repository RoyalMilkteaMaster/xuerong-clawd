...
describe("session HUD visual shell", () => {
  ...
  it("shows truthful work progress and the current working location", () => {
    assert.match(sessionHudHtml, /\.row\s*\{[\s\S]*height:\s*36px;[\s\S]*padding:\s*0 10px 7px;/);
    assert.match(sessionHudHtml, /\.work-progress\s*\{/);
    assert.match(sessionHudHtml, /\.progress-running\s*\{[\s\S]*animation:\s*work-progress-slide/);
    assert.match(sessionHudHtml, /\.progress-complete\s*\{[\s\S]*width:\s*100%;/);
    assert.match(sessionHudRenderer, /function locationFor\(session\)/);
    assert.match(sessionHudRenderer, /return `\$\{location\} · \$\{title\}`/);
    assert.match(sessionHudRenderer, /session\.badge === "done"[\s\S]*sessionBadgeDone[\s\S]*100%/);
    assert.match(sessionHudRenderer, /session\.badge === "running" \|\| session\.badge === "done"/);
    assert.match(sessionHudRenderer, /dashboardContextUsage[\s\S]*\$\{percent\}%/);
  });
});
