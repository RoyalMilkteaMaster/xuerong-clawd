...
test("pet interaction IPC settles a mini peek before taking the drag snapshot", () => {
...
test("pet interaction IPC can re-enter mini when a detached drag ends at an edge", () => {
  const { ipcMain, calls } = createHarness({
    state: { miniMode: true },
    checkMiniModeSnap: ({ calls, state }) => {
      calls.push(["checkMiniModeSnap"]);
...
test("pet interaction IPC accepts a detach while mini entry is still transitioning", () => {
  const { ipcMain, calls, state } = createHarness({
    state: { miniMode: true, miniTransitioning: true },
  });

  ipcMain.send("detach-mini-for-drag");

  assert.strictEqual(state.miniMode, false);
  assert.strictEqual(state.miniTransitioning, false);
  assert.deepStrictEqual(calls, [["exitMiniModeForDrag"]]);
});

test("pet interaction IPC persists an in-place mini drag", () => {
  const { ipcMain, calls } = createHarness({
    state: { miniMode: true },
  });
