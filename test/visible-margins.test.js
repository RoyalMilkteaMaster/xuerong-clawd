...
describe("edge pinning margin policy", () => {
  ...
  it("allows half of the visible pet to cross either horizontal screen edge", () => {
    const margins = getLooseDragMargins({
      width: 512,
      height: 512,
      visibleMargins: { left: 72, right: 72, top: 16, bottom: 24 },
      allowEdgePinning: false,
    });

    assert.strictEqual(margins.marginLeft, 256);
    assert.strictEqual(margins.marginRight, 256);
  });
