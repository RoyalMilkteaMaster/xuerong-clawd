...
describe("CodexLogMonitor", () => {
  ...
  it("emits a structured pending choice and clears it after the matching answer", (_, done) => {
    const testFile = path.join(dateDir, TEST_FILENAME);
    const requestPayload = {
      type: "function_call",
      name: "request_user_input",
      call_id: "choice-call-1",
      arguments: JSON.stringify({
        questions: [{
          ...
          options: [
            ...
            { label: "Safe", description: "Backed up" },
          ],
        }],
      }),
    };
    fs.writeFileSync(testFile, [
      ...
      JSON.stringify({ type: "response_item", payload: requestPayload }),
      JSON.stringify({
        type: "response_item",
        payload: { type: "function_call_output", call_id: "choice-call-1", output: "selected" },
      }),
    ].join("\n") + "\n");
    ...
    monitor = new CodexLogMonitor(config, (_sid, state, event, extra) => {
      ...
      if (event !== "response_item:request_user_input_output") return;
      assert.strictEqual(events[1].state, "codex-user-input");
      assert.strictEqual(events[1].event, "response_item:request_user_input");
      assert.strictEqual(events[1].extra.userInputRequest.questions[0].options[1].label, "Safe");
      assert.strictEqual(state, "working");
      assert.strictEqual(extra.userInputRequest, null);
      done();
    });
    monitor.start();
  });
