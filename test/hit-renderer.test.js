...
describe("hit-renderer input layer", () => {
  ...
  it("pulls inward from the right edge to detach and continue a normal drag", () => {
    const h = createHarness({ sendState: {
      currentState: "idle", miniMode: true, miniEdge: "right", dndEnabled: false,
    } });
    h.pointerdown({ clientX: 100, clientY: 100 });
    h.pointermove({ clientX: 96, clientY: 105 });
    h.pointermove({ clientX: 87, clientY: 106 });

    assert.deepStrictEqual(
      h.apiCalls.filter((call) => ["detachMiniForDrag", "startDragReaction"].includes(call[0])),
      [["detachMiniForDrag"], ["startDragReaction", "left"]],
    );
  });

  it("pulls inward from the left edge in the mirrored direction", () => {
    const h = createHarness({ sendState: {
      currentState: "idle", miniMode: true, miniEdge: "left", dndEnabled: false,
    } });
    h.pointerdown({ clientX: 100, clientY: 100 });
    h.pointermove({ clientX: 113, clientY: 100 });

    assert.deepStrictEqual(
      h.apiCalls.filter((call) => ["detachMiniForDrag", "startDragReaction"].includes(call[0])),
      [["detachMiniForDrag"], ["startDragReaction", "right"]],
    );
  });

  it("unlocks X even when the drag starts toward the outside edge", () => {
    const h = createHarness({ sendState: {
      currentState: "idle", miniMode: true, miniEdge: "right", dndEnabled: false,
    } });
    h.pointerdown({ clientX: 100, clientY: 100 });
    h.pointermove({ clientX: 112, clientY: 140 });

    assert.deepStrictEqual(h.apiCalls.filter((call) => call[0] === "detachMiniForDrag"), [["detachMiniForDrag"]]);
    assert.deepStrictEqual(h.apiCalls.filter((call) => call[0] === "startDragReaction"), [["startDragReaction", "right"]]);
  });

  it("uses stable screen coordinates when the hit window's client coordinates shift", () => {
    const h = createHarness({ sendState: {
      currentState: "idle", miniMode: true, miniEdge: "right", dndEnabled: false,
    } });
