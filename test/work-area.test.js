...
describe("computeLooseClamp", () => {
  ...
  it("supports separate visible-content overflow on the left and right", () => {
    const displays = [display(0, 0, 1920, 1080)];
    const left = computeLooseClamp(displays, null, -500, 100, 512, 512, {
      marginLeft: 256,
      marginRight: 220,
    });
    const right = computeLooseClamp(displays, null, 5000, 100, 512, 512, {
      marginLeft: 256,
      marginRight: 220,
    });

    assert.strictEqual(left.x, -256);
    assert.strictEqual(right.x, 1628);
  });
