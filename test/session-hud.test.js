...
describe("session HUD layout", () => {
  ...
  it("computeHudHeight multiplies row count by row height", () => {
    ...
    assert.strictEqual(
      computeHudHeight(2, constants.HUD_USER_INPUT_CARD_HEIGHT),
      constants.HUD_ROW_HEIGHT * 2
        + constants.HUD_BORDER_Y
        + constants.HUD_USER_INPUT_CARD_HEIGHT
    );
  });
});
...
describe("session HUD auto-hide helpers", () => {
  ...
  it("keeps a pending Codex choice visible even when the HUD is unpinned", () => {
    const r = evaluateShouldShow({
      ...baseFlags,
      snapshot: {
        sessions: [mkSession("codex:1", {
          state: "idle",
          userInputRequest: { questions: [{ question: "Choose", options: [{ label: "A" }, { label: "B" }] }] },
        })],
      },
      clickRevealed: false,
      sessionHudPinned: false,
