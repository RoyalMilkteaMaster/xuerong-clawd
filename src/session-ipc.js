...
function registerSessionIpc(options = {}) {
  ...
  const clipboard = requiredDependency(options.clipboard, "clipboard");
  const disposers = [];
  ...
  on("session-hud:open-user-input-session", (_event, sessionId) => {
  ...
  handle("session-hud:choose-user-input-option", (_event, payload) => {
    const sessionId = payload && typeof payload.sessionId === "string" ? payload.sessionId : "";
    const label = payload && typeof payload.label === "string"
      ? payload.label.replace(/[\r\n\t]+/g, " ").trim().slice(0, 200)
      : "";
    if (!sessionId || !label) {
      return { status: "error", message: "sessionId and label are required" };
    }
    try {
      clipboard.writeText(label);
    } catch (err) {
      return { status: "error", message: err && err.message ? err.message : "clipboard write failed" };
    }
    const focused = focusSession(sessionId, {
      requestSource: "hud-user-input-choice",
      allowCodexDesktopFallback: true,
    });
    return { status: "ok", copied: true, focused: focused === true };
  });
