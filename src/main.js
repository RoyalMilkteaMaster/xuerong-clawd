...
const petWindowRuntime = createPetWindowRuntime({
  ...
  moveMiniWindowForDrag: (snapshot, cursor) => _mini.moveWindowForDrag(snapshot, cursor),
...
const CODEX_WINDOWS_APP_ID = "OpenAI.Codex_2p2nqsd0c76g0!App";

function focusCodexDesktopApp(requestSource, sessionId) {
  if (process.platform !== "win32") return false;
  try {
    const child = require("child_process").spawn(
      "explorer.exe",
      [`shell:AppsFolder\\${CODEX_WINDOWS_APP_ID}`],
      { detached: true, stdio: "ignore", windowsHide: true }
    );
    child.on("error", (err) => {
      focusLog(`focus Codex desktop error source=${requestSource} sid=${sessionId} message=${err && err.message}`);
    });
    child.unref();
    focusLog(`focus result branch=codex-desktop-app source=${requestSource} sid=${sessionId}`);
    return true;
  } catch (err) {
    focusLog(`focus result branch=none reason=codex-desktop-launch-failed source=${requestSource} sid=${sessionId}`);
    return false;
  }
}
...
function focusDashboardSession(sessionId, options = {}) {
  ...
  if (options.allowCodexDesktopFallback === true && getCodexThreadId(focusEntry)) {
    return focusCodexDesktopApp(requestSource, id);
  }
...
function createWindow() {
  ...
  registerPetInteractionIpc({
    ...
    repositionFloatingBubbles: () => repositionFloatingBubbles(),
    exitMiniMode: () => exitMiniMode(),
    exitMiniModeForDrag: () => _mini.exitMiniModeForDrag(),
    prepareMiniDrag: () => _mini.prepareForDrag(),
