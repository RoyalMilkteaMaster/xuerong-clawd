...
const HUD_ROW_HEIGHT = 36;
const HUD_USER_INPUT_CARD_HEIGHT = 206;
...
function snapshotHasPendingUserInput(snapshot) {
  const sessions = Array.isArray(snapshot && snapshot.sessions) ? snapshot.sessions : [];
  return sessions.some((session) => isHudSession(session) && session.userInputRequest);
}
...
function evaluateShouldShow({
  ...
  if (snapshotHasPendingUserInput(snapshot)) return { show: true, nextHoldUntil: 0 };
...
function computeHudHeight(rowCount, extraHeight = 0) {
  if (!Number.isFinite(rowCount) || rowCount <= 0) return HUD_ROW_HEIGHT;
  const extra = Number.isFinite(extraHeight) && extraHeight > 0 ? extraHeight : 0;
  return rowCount * HUD_ROW_HEIGHT + HUD_BORDER_Y + extra;
}

function getHudUserInputCardHeight(layout) {
  const expanded = layout && Array.isArray(layout.expanded) ? layout.expanded : [];
  return expanded.some((session) => session && session.userInputRequest)
    ? HUD_USER_INPUT_CARD_HEIGHT
    : 0;
}
...
module.exports = function initSessionHud(ctx) {
  ...
  function shouldShow(snapshot = latestSnapshot) {
    ...
    if (snapshotHasPendingUserInput(snapshot)) return true;
  ...
  function isAutoHidePollingNeeded() {
    ...
    if (snapshotHasPendingUserInput(latestSnapshot)) return false;
  ...
  function computeExpectedHudContentBounds(snapshot, scale = getTextScale()) {
    ...
    const height = computeHudHeight(layout.rowCount, getHudUserInputCardHeight(layout));
