  ...
  function startCodexLogMonitor() {
    ...
    try {
      ...
      codexMonitor = new CodexLogMonitor(codexAgent, (sid, state, event, extra) => {
        ...
        if (isCodexMonitorUserInputEvent(state)) {
          clearCodexNotifyBubbles(sid, "codex-user-input");
          updateSession(sid, "attention", event, buildCodexMonitorUpdateOptions(extra, {
            includeHeadless: true,
          }));
          return;
        }
