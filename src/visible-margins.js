...
function getLooseDragMargins({ width, height, visibleMargins, allowEdgePinning, bottomInset } = {}) {
  ...
  const widthPx = Number.isFinite(width) ? Math.max(0, Math.round(width)) : 0;
  const leftMargin = normalizeMargin(margins.left);
  const rightMargin = normalizeMargin(margins.right);
  const visibleWidth = Math.max(0, widthPx - leftMargin - rightMargin);
  // Dragging should follow the visible pet, not the transparent render window.
  // Permit exactly half of the visible character to cross an outer display edge.
  const halfVisibleWidth = Math.round(visibleWidth * 0.5);
  const marginLeft = Math.min(widthPx, leftMargin + halfVisibleWidth);
  const marginRight = Math.min(widthPx, rightMargin + halfVisibleWidth);
