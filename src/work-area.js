...
function computeLooseClamp(displays, primaryWa, x, y, w, h, options = {}) {
  ...
  const fallbackMarginX = options.marginX != null ? options.marginX : Math.round(w * 0.25);
  const marginLeft = options.marginLeft != null ? options.marginLeft : fallbackMarginX;
  const marginRight = options.marginRight != null ? options.marginRight : fallbackMarginX;
  ...
  return {
    x: Math.max(minX - marginLeft, Math.min(x, maxX - w + marginRight)),
