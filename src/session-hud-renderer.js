...
function locationFor(session) {
  const cwd = String((session && session.cwd) || "").replace(/[\\/]+$/, "");
  if (!cwd) return "";
  const segments = cwd.split(/[\\/]/).filter(Boolean);
  return segments.length ? segments[segments.length - 1] : cwd;
}

function hudTitleFor(session) {
  const title = titleFor(session);
  const location = locationFor(session);
  if (!location || location === title) return title;
...
function stateChipInfo(session) {
  ...
  if (session && session.userInputRequest) {
    return { label: t("sessionHudNeedsChoice"), cls: "chip-notification" };
  }
...
function hasUserInputRequest(session) {
  const request = session && session.userInputRequest;
  return !!(request && Array.isArray(request.questions) && request.questions.length > 0);
}

function createUserInputCard(session) {
  const card = document.createElement("section");
  card.className = "user-input-card";
  card.setAttribute("aria-label", t("sessionHudNeedsChoice"));
  card.setAttribute("aria-live", "polite");

  const heading = document.createElement("div");
  heading.className = "user-input-heading";
  heading.textContent = t("sessionHudNeedsChoice");
  card.appendChild(heading);

  for (const question of session.userInputRequest.questions) {
    const group = document.createElement("div");
    group.className = "user-input-question";
    ...
    for (const option of question.options) {
      ...
      button.addEventListener("click", (event) => {
        ...
        Promise.resolve(
          window.sessionHudAPI.chooseUserInputOption(session.id, option.label),
        ).catch((error) => {
          console.warn("Failed to copy Codex choice:", error);
        });
      });
  ...
  note.textContent = t("sessionHudChoiceCopyHint");
  footer.appendChild(note);

  const openButton = document.createElement("button");
  openButton.type = "button";
  openButton.className = "user-input-open";
  openButton.textContent = t("sessionHudOpenCodex");
  openButton.addEventListener("click", (event) => {
    event.stopPropagation();
    window.sessionHudAPI.openUserInputSession(session.id);
  });
...
function usageChipInfo(session) {
  ...
  if (percentKnown) {
    ...
    return {
      label: `${t("dashboardContextUsage")} ${percent}%`,
...
function createRowForSession(session, now) {
  ...
  const fullTitle = hudTitleFor(session);
  const shortTitle = shortenHudTitle(fullTitle);
  title.className = "title";
  title.textContent = shortTitle;
  const fullPath = String(session.cwd || "");
  if (fullPath) title.title = `${fullTitle}\n${fullPath}`;
  else if (shortTitle && shortTitle !== fullTitle) title.title = fullTitle;
  ...
  if (session.badge === "done") {
    const completion = document.createElement("span");
    completion.className = "completion-progress-label";
    completion.textContent = `${t("sessionBadgeDone")} 100%`;
    right.appendChild(completion);
    hasRightContent = true;
  }
  ...
  if (session.badge === "running" || session.badge === "done") {
    const progress = document.createElement("span");
    progress.className = "work-progress";
    progress.setAttribute("role", "progressbar");
    progress.setAttribute("aria-label", session.badge === "done"
      ? `${t("sessionBadgeDone")} 100%`
      : t("sessionBadgeRunning"));
    if (session.badge === "done") {
    ...
    fill.className = `work-progress-fill ${session.badge === "done" ? "progress-complete" : "progress-running"}`;
...
function render() {
  ...
  hudEl.classList.toggle("has-input", expandedUserInputExists(sessions));
  if (!sessions.length) return;

  const now = Date.now();
  const { expanded, folded } = splitHudLayout(sessions);

  let inputCardRendered = false;
  for (const session of expanded) {
    hudEl.appendChild(createRowForSession(session, now));
    if (!inputCardRendered && hasUserInputRequest(session)) {
      hudEl.appendChild(createUserInputCard(session));
      inputCardRendered = true;
    }
  }
  if (folded.length > 0) {
...
function expandedUserInputExists(sessions) {
  const maxRows = snapshot.hudShowStateLabels === false
    ? HUD_MAX_EXPANDED_ROWS
    : HUD_MAX_EXPANDED_ROWS_LABELS;
  return sessions.slice(0, maxRows).some(hasUserInputRequest);
}
