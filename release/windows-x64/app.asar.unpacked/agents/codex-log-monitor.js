...
const BACKFILL_SNAPSHOT_STATES = new Set(["thinking", "working", "codex-permission", "codex-user-input"]);
...
class CodexLogMonitor {
  ...
  _pollFile(filePath, fileName) {
    ...
    if (!tracked) {
      ...
      tracked = {
        ...
        pendingUserInputRequest: retired ? retired.pendingUserInputRequest || null : null,
  ...
  _processLine(line, tracked) {
    ...
    const userInputRequest = extractCodexUserInputRequest(payload);
    const resolvesUserInput = isMatchingCodexUserInputOutput(
      payload,
      tracked.pendingUserInputRequest
    );
    let clearUserInputRequest = false;
    ...
    if (resolvesUserInput) {
      tracked.pendingUserInputRequest = null;
      tracked.lastStateEvent = "response_item:request_user_input_output";
      ...
      this._emitStateChange(
        tracked,
        "working",
        "response_item:request_user_input_output",
    ...
    const state = userInputRequest ? "codex-user-input" : map[key];
    if (state === undefined) return; // unmapped event, skip
    if (state === null) return; // explicitly ignored
    tracked.lastStateEvent = userInputRequest ? "response_item:request_user_input" : key;

    // Track tool use per turn — reset on task_started, set on function_call
    if (key === "event_msg:task_started") {
      clearUserInputRequest = !!tracked.pendingUserInputRequest;
      tracked.pendingUserInputRequest = null;
    ...
    if (userInputRequest) {
      tracked.pendingUserInputRequest = userInputRequest;
      if (tracked.backfilling) {
        tracked.lastState = "codex-user-input";
        return;
      }
      this._emitStateChange(
        tracked,
        "codex-user-input",
        "response_item:request_user_input",
        { userInputRequest }
      );
      return;
    }
    ...
    if (state === "codex-turn-end") {
      clearUserInputRequest = clearUserInputRequest || !!tracked.pendingUserInputRequest;
      tracked.pendingUserInputRequest = null;
      if (tracked.approvalTimer) {
        clearTimeout(tracked.approvalTimer);
        tracked.approvalTimer = null;
      ...
      this._emitStateChange(tracked, resolved, key, {
        ...
        ...(clearUserInputRequest ? { userInputRequest: null } : {}),
      });
      return;
    }
    ...
    this._emitStateChange(
      ...
      state,
      key,
      clearUserInputRequest ? { userInputRequest: null } : null
    );
  }
  ...
  _retireTrackedFile(filePath, tracked) {
    ...
    this._retiredTracked.set(filePath, {
      ...
      pendingUserInputRequest: tracked.pendingUserInputRequest || null,
    });
  ...
  _emitBackfillSnapshot(tracked) {
    ...
    } else if (snapshotState === "codex-user-input" && tracked.pendingUserInputRequest) {
      extra = { userInputRequest: tracked.pendingUserInputRequest };
    }
    this._emitStateChange(
      tracked,
      snapshotState,
